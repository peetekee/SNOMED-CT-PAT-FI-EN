        ...
        for _, en_row in self.__activated_en_rows.iterrows():
            ...
            for _, lang_row in lang_rows.iterrows():
                ...
                self.__database = Put.inactivated_row(
                    self.__database, index, en_row[COLUMNS["beginning_date"]],en_row[COLUMNS["expiring_date"]], en_row[COLUMNS["inaktivoinnin_selite"]], en_row[COLUMNS["edit_comment"]])

            # Inactivate en_row
            index = Get.index_by_codeid(
                self.__database, en_row[COLUMNS["code_id"]])
            self.__database = Put.inactivated_row(
                self.__database, index, en_row[COLUMNS["beginning_date"]], en_row[COLUMNS["expiring_date"]], en_row[COLUMNS["edit_comment"]])
